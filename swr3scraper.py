...
def getSWR3Playlist(plURL):
    ...
    while(i == 1 or len(elDate) > 0):
        ...
        elTitle  = soup.select('#playlist > li:nth-of-type(' + str(i) + ') > div > div.detail-body > h4')
...
begDate = datetime.date(2016, 4, 1)
...
i = 0
with open('swr3-songs-' + endDate.strftime('%y%m%d') + '-' + begDate.strftime('%y%m%d') + '.csv', 'w') as tgtFile:
